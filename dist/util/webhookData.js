...
async function createTemporaryBatcher(id, data, options) {
    ...
    if (exports.batches.has(id))
        return exports.batches.get(id).add(data);
...
class WebhookData {
    ...
    isChildAction() {
        return (this.action.type.replace(/[A-Z]/g, (letter) => `_${letter.toLowerCase()}`).toUpperCase() !==
            this.filterFlag);
    }
    get body() {
    ...
    embedDescription(fields = null) {
        ...
        const lines = {
            invoker: `**${_('words.member.one')}:** [${this.invoker.fullName
                ...
                : this.invoker.username}](https://trello.com/${this.invoker.username}?utm_source=tacobot.app)`,
            member: this.member
                ? `**${_('words.member.one')}:** [${this.member.fullName
                    ? `${(0, _1.cutoffText)(this.member.fullName, 50)} (${this.member.username})`
                    : this.member.username}](https://trello.com/${this.member.username}?utm_source=tacobot.app)`
                : '',
            card: this.card && this.card.name
                ? `**${_('words.card.one')}:** [${(0, _1.cutoffText)((0, _1.escapeMarkdown)(this.card.name), 50)}](https://trello.com/c/${this.card.shortLink}?utm_source=tacobot.app)`
                : '',
            ...
            listBefore: this.listBefore
                ? `**${_('trello.prev_list')}:** ${(0, _1.cutoffText)((0, _1.escapeMarkdown)(this.listBefore.name), 50)}`
                : '',
            listAfter: this.listAfter
                ? `**${_('trello.curr_list')}:** ${(0, _1.cutoffText)((0, _1.escapeMarkdown)(this.listAfter.name), 50)}`
                : '',
            checklist: this.checklist && this.checklist.name
                ? `**${_('words.checklist.one')}:** ${(0, _1.cutoffText)((0, _1.escapeMarkdown)(this.checklist.name), 50)}`
                : '',
            checklistItem: this.checklistItem && this.checklistItem.name
                ? `**${_('words.checklist_item.one')}:** ${(0, _1.cutoffText)((0, _1.escapeMarkdown)(this.checklistItem.name), 50)}`
                : '',
            customField: this.customField && this.customField.type
                ? `**${_('trello.custom_field')} (${_(`custom_field_types.${this.customField.type}`)}):** ${(0, _1.cutoffText)((0, _1.escapeMarkdown)(this.customField.name), 50)}`
                : '',
            label: this.label && this.label.name
                ? `**${_('words.label.one')}${this.label.color ? ` (${_(`trello.label_color.${this.label.color}`)})` : ''}:** ${(0, _1.cutoffText)((0, _1.escapeMarkdown)(this.label.name), 50)}`
                : '',
            attachment: this.attachment && this.attachment.name
                ? `**${_('words.attachment.one')}:** ${this.attachment.url
                    ? `[${(0, _1.cutoffText)((0, _1.escapeMarkdown)(this.attachment.name), 50)}](${this.attachment.url})`
    ...
    async send(embedStyles) {
        ...
        const EMBED_DEFAULTS = {
            default: {
                color: this.isChildAction() ? exports.DEFAULT_COLORS.CHILD : exports.DEFAULT_COLORS[this.filterFlag.split('_')[0]],
                author: {
        ...
        if (this.webhook.style === 'compact') {
            ...
            const compactLine = `\`${this.isChildAction() ? exports.COMPACT_EMOJIS.CHILD : exports.COMPACT_EMOJIS[this.filterFlag.split('_')[0]]}\` ${embedStyles.small.description}`;
            createTemporaryBatcher(batchKey, compactLine, {
                maxTime: 2000,
                maxSize: 10,
                onBatch: (lines) => {
                    createTemporaryBatcher(this.webhook.webhookID, lodash_1.default.defaultsDeep({ description: lines.join('\n') }, EMBED_DEFAULTS.compact), {
                        maxTime: 1000,
                        maxSize: 10,
                        onBatch: (embeds) => {
                            (0, influx_1.onWebhookSend)(this.webhook.webhookID);
                            logger_1.logger.info('Posting webhook %s (guild=%s, time=%d)', this.webhook.webhookID, this.webhook.guildID, Date.now());
        ...
        return createTemporaryBatcher(this.webhook.webhookID, lodash_1.default.defaultsDeep(embedStyles[this.webhook.style], EMBED_DEFAULTS[this.webhook.style]), {
            maxTime: 1000,
            maxSize: 10,
            onBatch: (embeds) => {
                (0, influx_1.onWebhookSend)(this.webhook.webhookID);
        ...
        try {
            ...
            if (e.name.startsWith('DiscordRESTError')) {
                if (e.code === 10015) {
                    ...
                    logger_1.logger.warn(`Discord webhook token invalid, dropping @ ${this.webhook.webhookID}:${this.webhook.id}`, e);
                    await postgres_1.Webhook.update({
                        webhookID: null,
                        webhookToken: null
                    }, { where: { id: this.webhook.id } });
                    ...
                    logger_1.logger.error(`Invalid form body, dropping @ ${this.webhook.webhookID}:${this.webhook.id} - ${this.filterFlag}`, e);
                }
                else {
                    attempt++;
                    if (attempt > 3) {
                        logger_1.logger.error(`Discord Error ${e.code} (${e.status}), exceeded attempts, dropping @ ${this.webhook.webhookID}:${this.webhook.id}`, e);
                    }
                    else {
                        logger_1.logger.warn(`Discord Error ${e.code} (${e.status}), retrying (${attempt}) @ ${this.webhook.webhookID}:${this.webhook.id}`);
                ...
                attempt++;
                if (attempt < 3) {
                    logger_1.logger.error(`Discord server error, exceeded attempts, dropping @ ${this.webhook.webhookID}:${this.webhook.id}`);
                }
                else {
                    logger_1.logger.warn(`Discord server error, retrying (${attempt}) @ ${this.webhook.webhookID}:${this.webhook.id}`);
                    return this._send(embeds, attempt);
                }
            }
                ...
                attempt++;
                if (attempt < 3) {
                    logger_1.logger.error(`Request timed out, exceeded attempts, dropping @ ${this.webhook.webhookID}:${this.webhook.id}`);
                }
                else {
                    logger_1.logger.warn(`Request timed out, retrying (${attempt}) @ ${this.webhook.webhookID}:${this.webhook.id}`);
                    return this._send(embeds, attempt);
                }
            }
            else {
